...
**配图强制（2026-09-03 起）**：每篇必须有封面 1 张，正文每个一级小节（## 标题）至少 1 张非封面配图（同一张图不跨小节复用；「本节难点/要点速记/动手任务/完成标志/常见问题 Q&A/参考/下一步/延伸阅读/行动号召/配图登记」等辅助或收尾小节不计）。配图来源按下表，**同一篇可混用多种来源**（混用规则见下文「来源混用与效率原则」）。

| 场景 | 方式 | 成本 | 特点 |
| --- | --- | --- | --- |
| 系列封面、每课封面、公众号头图、正文知识/对比/流程卡 | 模板化配图（主视觉推荐） | 零（无 AI 生图费用） | 封面与模板卡同篇统一、可控、可批量 |
...
- **允许混用**：模板化配图、Mermaid、实拍截图、AI 生图可以在同一篇文章里并存——模板卡负责"主视觉骨架"，其余来源按内容需要补充，不要求全篇所有图片出自同一通道；
- **模板内部保持统一**：由模板生成的封面与正文知识卡仍共用同一 `--style`（slug 记入 frontmatter `style`），保证主视觉一致；非模板来源（截图/Mermaid/AI 图）不强制套模板配色，但避免与主风格明显打架（AI 生图提示词可参考主风格的关键词）；
- **按内容选最省力的来源**：能截图的用真实截图（零成本、证据力强）；流程/结构拆解优先 Mermaid 或 card-flow/card-list；知识对比用 card-compare/card-knowledge；只有需要手绘感或照片氛围时才走 AI 生图——先穷尽零成本通道，再考虑付费生图；
- **效率优先级**：同批出图用 `generate.mjs`/批量渲染脚本一次生成；Mermaid 在 Obsidian 直接渲染、发布前转 PNG；AI 生图单张按需调用，不为凑配图而铺张。

## 方案一：模板化配图（推荐，通用 HTML→PNG 引擎）

### 原理

HTML/CSS 模板 + 无头浏览器截图：把标题、副标题、分类、期号等填入模板，渲染成统一风格的 PNG。零生成成本、完全可控，适合「批量出图、主视觉统一」的场景——公众号封面、知识库头图、每课封面，以及文章里的知识卡、对比图、示意图等正文配图。模板体系内部同篇一致（封面与模板卡共用同一 `--style`）；模板之外的真实截图、Mermaid、AI 生图可与模板卡混用（见上文「来源混用与效率原则」）。篇与篇默认随机换风格，指定 `--style` 则全按指定。

### 使用

示例默认在技能根目录执行；文章位于 `<项目根>/generated/<英文标题>/`，下文示例里的 Markdown/输出路径按实际位置补齐（如 `--input "generated/<英文标题>/01-...md"`），不强制抄写目录前缀。
...
**模板风格统一（模板体系内硬性约定）**：随机发生在「篇」粒度——同一篇文章（同一 lesson）由模板生成的图片（封面 + 正文知识卡）必须使用同一个 `--style` slug。流程：封面首次生成时随机定下 slug（或由用户指定），把该 slug 记入文章 frontmatter 的 `style` 字段，正文模板卡生成时沿用同一 slug；figures 正文卡通过 `{{PALETTE_BG/INK/SUB/ACCENT}}` 自动继承该风格配色，保证模板主视觉统一。实拍截图、Mermaid 转出的 PNG、AI 生图等非模板来源可混入正文（不与模板共用 style），但应在版面密度与色彩上避免与主视觉冲突。
...
- 「系列目录」即 SKILL.md「输出目录约定」里的 `<项目根>/generated/<英文标题>/`，下文所有 `<系列目录>` 均同义；
...
- 文件名：`NN-小节号-用途`，建议与正文小节一一对应，如 `05-01-规则手册目录.png`；简短、语义清晰，可混用中英文；
- 每篇正文的每个一级小节都必须引用一张本目录下的配图，同一张图不跨小节复用；
...
- [ ] 模板主视觉统一：同篇封面与模板卡使用同一 --style（默认每篇随机，指定则按指定；slug 已记入 frontmatter style 字段）；非模板来源（截图/Mermaid/AI 图）可混用且与主视觉不冲突
- [ ] 正文每个一级小节至少一张配图，无缺小节图（check-images.mjs 会强制）
